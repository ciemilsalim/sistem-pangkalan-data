        ...
        >
            ...
            <div className="py-6 space-y-6">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6 rounded-r-xl border-l-4 border-blue-500 bg-blue-50 p-4 shadow-sm">
                        <div className="flex">
                    ...
                    <div className="flex border-b border-gray-200 mb-6 bg-white p-1 rounded-xl shadow-sm max-w-2xl">
                        <button
                    ...
                    {activeTab === 'kbm' && (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                <div className="rounded-xl bg-white p-5 shadow-sm border border-gray-205 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <h4 className="text-xl font-bold text-gray-800 font-mono mt-0.5">{totalAssignments} Kelas</h4>
                                    </div>
                                </div>
                                <div className="rounded-xl bg-white p-5 shadow-sm border border-gray-205 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <h4 className="text-xl font-bold text-emerald-600 font-mono mt-0.5">{completedCompliance} Kelas ({totalAssignments > 0 ? round((completedCompliance / totalAssignments) * 100) : 0}%)</h4>
                                    </div>
                                </div>
                                <div className="rounded-xl bg-white p-5 shadow-sm border border-gray-205 flex items-center gap-4">
                                        ...
                                        <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                                            {kbm_audit.map((row) => (
                                                <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{row.teacher?.name}</td>
                                                    ...
                                                    <td className="px-6 py-4 text-center font-mono font-bold text-gray-800">{row.materials_count}</td>
                                                    <td className="px-6 py-4 text-center font-mono font-bold text-gray-800">{row.assignments_count}</td>
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                                        {remedial_audit.map((row) => (
                                            <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                                                <td className="px-6 py-4 font-semibold text-gray-900">{row.student_name}</td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-center font-mono font-bold text-emerald-650">
                                                    {row.remedial_score !== null ? row.remedial_score : '-'}
                                                </td>
                                                <td className="px-6 py-4 text-xs font-medium text-gray-500">{row.strategy}</td>
                                                <td className="px-6 py-4 text-right">
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
                                        {diagnostic_audit.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-xs text-gray-500 truncate max-w-[150px]" title={row.interests}>
                                                    {row.interests}
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`font-mono font-bold text-sm ${row.is_passed === true ? 'text-emerald-650' : row.is_passed === false ? 'text-rose-650' : 'text-gray-500'}`}>
                                                        {row.cognitive_score !== null ? row.cognitive_score : '-'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-xs text-gray-500 max-w-xs leading-relaxed italic" title={row.recommendation}>
                                                    {row.recommendation}
                                                </td>
                                            </tr>
                                        ))}
